...
export function removeKnownRegion(content: string, locale: string): { content: string; updated: boolean } {
  const normalizedLocale = formatLocaleCode(locale)
  ...
  const normalizedTarget = normalizedLocale.toLowerCase()
  const nextTokens = tokens.filter((token) => token.toLowerCase() !== normalizedTarget)
  ...
  const closingIndent = closingIndentMatch
    ...
    : itemIndent.slice(0, Math.max(itemIndent.length - 1, 0))
