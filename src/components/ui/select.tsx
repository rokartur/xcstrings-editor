import { ArrowDownIcon, ArrowUpIcon, CheckIcon, ChevronUpDownIcon } from '@/components/icons'
...
function SelectValue({ className, placeholder, ...props }: SelectPrimitive.Value.Props & { placeholder?: string }) {
	return (
		<SelectPrimitive.Value
			...
			className={cn(
				'flex flex-1 text-left data-[placeholder]:text-muted-foreground data-[placeholder]:after:content-[attr(data-placeholder-text)]',
				className,
			)}
...
function SelectTrigger({
	...
	return (
		<SelectPrimitive.Trigger
			...
			data-size={size}
			className={cn(
				"flex w-fit items-center justify-between gap-1.5 rounded-md border border-input bg-input/20 px-2 py-1.5 text-xs/relaxed whitespace-nowrap transition-colors outline-none focus-visible:border-ring focus-visible:ring-[2px] focus-visible:ring-ring/30 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-[2px] aria-invalid:ring-destructive/20 data-[placeholder]:text-muted-foreground data-[size=default]:h-7 data-[size=sm]:h-6 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 dark:bg-input/30 dark:hover:bg-input/50 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
				className,
			)}
			{...props}
		>
			...
			<SelectPrimitive.Icon
				render={<ChevronUpDownIcon className='pointer-events-none size-3.5 text-muted-foreground' />}
			/>
		</SelectPrimitive.Trigger>
	)
}
